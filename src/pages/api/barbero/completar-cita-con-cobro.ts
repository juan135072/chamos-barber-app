...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (typeof monto_cobrado !== 'number' || !Number.isFinite(monto_cobrado) || monto_cobrado <= 0 || monto_cobrado > 100000000 || !PAYMENT_METHODS.includes(metodo_pago)) {
  ...
  try {
    ...
    const { error: saleError } = await actor.admin.rpc('app_record_sale', { p_data: {
      ...
      items: Array.isArray(cita.items) && cita.items.length ? cita.items : [{ servicio_id: cita.servicio_id, nombre: cita.servicios?.nombre, cantidad: 1, precio: monto_cobrado, subtotal: monto_cobrado }],
      notas_tecnicas, foto_resultado_url,
    } })
    if (saleError) return respondPosError(res, saleError, 'cobro-barbero')
